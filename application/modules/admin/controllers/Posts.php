...
class Posts extends MY_Controller {
	...
	public function __construct(){
        ...
        $this->load->model('Tag');
        $this->allow_group_access(array('admin','editor-english','editor-arabic'));
	...
	public function index(){
        ...
        if ($this->input->get('q')):
            $q = $this->input->get('q');
            if($this->ion_auth->in_group(array('editor-arabic'))){
            $this->data['posts_arabic'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE, $q,array('ar'));
            }else{
            $this->data['posts_arabic'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE, $q,array('ar'));
            $this->data['posts'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE, $q,array('en'));
            }
            if (empty($this->data['posts'])) {
                $this->session->set_flashdata('message', message_box('Data Not Available','danger'));
                redirect('admin/posts/index');
            }
            $config['total_rows'] = count($this->data['posts']);
        else:
             if($this->ion_auth->in_group(array('editor-arabic'))){
            $this->data['posts_arabic'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE,null,array('ar'));
            }else{
            $this->data['posts_arabic'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE, null,array('ar'));
            $this->data['posts'] = $this->Post->find($config['per_page'], $this->uri->segment(4),FALSE, null,array('en'));
            }
